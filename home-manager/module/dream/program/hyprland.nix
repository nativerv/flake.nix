...
with self.lib;
let
  cfg = config.dream.program.hyprland;
in
{
  options.dream.program.hyprland = {
    enable = mkEnableOption "Enable program.hyprland";
  };
      ...
      settings = lib.mkMerge [
        ...
        }
        {
          ...
          decoration = {
            ...
            shadow = {
              "enabled" = "true"; # [bool] [true] enable drop shadows on windows
              "range" = "4"; # [int] [4] Shadow range (“size”) in layout px
              "render_power" = "3"; # [int] [3] in what power to render the falloff (more power, the faster the falloff) [1 - 4]
              "sharp" = "false"; # [bool] [false] If enabled, will make the shadows sharp, akin to an infinite render power
              "ignore_window" = "true"; # [bool] [true] if true, the shadow will not be rendered behind the window itself, only around it.
              "color" = "0x1a1a1aee"; # [color] [0xee1a1a1a] shadow’s color. Alpha dictates shadow’s opacity.
              #"color_inactive" = "unset"; # [color] [unset] inactive shadow color. (if not set, will fall back to col.shadow)
              "offset" = "0, 0"; # [vec2] [0, 0] shadow’s rendering offset.
              "scale" = "1.0"; # [float] [1.0] shadow’s scale. [0.0 - 1.0]
            };
            blur = {
