...
divisor="${2:-4096}" #my ADATA SSD disk uses 4K units
...
sudo parted "${disk}" print >/dev/null || exit $?
partedoutput=`sudo parted "${disk}" unit B print`

echo "what does the parted utility say about ${disk} partitions alignment?"
partitions=`echo "$partedoutput" | grep -oE "^[[:blank:]]*[0-9]+"`
partederror=
#           ^null
for i in ${partitions}; do
  sudo parted "${disk}" align-check opt $i || partederror=true
done
