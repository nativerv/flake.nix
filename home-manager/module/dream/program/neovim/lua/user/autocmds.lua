...
-- Reverse search mappings in cmdwin
vim.api.nvim_create_autocmd({ 'CmdwinEnter', }, {
  ...
  callback = function()
    vim.keymap.set('n', '?', '/', { noremap = true, buffer = 0, })
    vim.keymap.set('n', '/', '?', { noremap = true, buffer = 0, })
  end,
})

-- Show bad things in statusbar on critical error
vim.api.nvim_create_autocmd({ 'WinEnter', 'BufEnter', }, {
  ...
  callback = function()
    if vim.g.bad_message == '' then return end
    vim.wo.statusline = '%!g:bad_message'
    vim.api.nvim_set_hl(0, 'StatusLine', { bg = '#AA0000', });
  end,
})

-- An attempt on immediately showing diff when swap exists:
vim.cmd [[
